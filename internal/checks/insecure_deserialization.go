...
func (InsecureDeserialization) Name() string { return "insecure-deserialization" }

func (InsecureDeserialization) Level() Level { return LevelDefault }
...
func (c InsecureDeserialization) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) ([]Finding, error) {
	...
	// Fingerprint arm: scan the baseline snapshot for serialized data in
	// places that round-trip back to the server. Reuses the crawler's
	// fetched body when available so this arm costs zero extra requests.
	base, baseErr := ensureResponse(ctx, client, p, insecDeserialBodyCap)
	if baseErr == nil && base.Headers != nil {
		c.scanFingerprints(ctx, p, u, base, add)
	}

	// Probe arm: per-sink malformed-payload sweep. Bounded by
	// len(SinksFor(p)) * (1 baseline + len(deserialFormats())) requests.
	for _, sink := range SinksFor(p) {
		...
		f, err := c.probeSink(ctx, client, p.URL, sink)
		if err != nil {
	...
	if LevelFrom(ctx) >= LevelAggressive && len(base.Body) > 0 {
		if marker := bodyDeserialMarker(base.Body); marker != "" {
			add(&Finding{
				...
				DedupeKey: MakeKey(c.Name(), ScopePage, p.URL, "body-fingerprint:"+marker),
			})
		}
	}
}
...
func (c InsecureDeserialization) fingerprintFinding(target, location string, fp *deserialFormat, value string, severity Severity) *Finding {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopePage, target, "fingerprint", "format:"+fp.name, "location:"+location),
	}
}
...
var _ Check = InsecureDeserialization{}

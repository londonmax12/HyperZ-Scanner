...
func (FormActionInsecure) Name() string { return "form-action-insecure" }

func (FormActionInsecure) Level() Level { return LevelPassive }
...
func (c FormActionInsecure) Run(ctx context.Context, client *httpclient.Client, _ *scope.Scope, p page.Page) ([]Finding, error) {
	// Only meaningful on HTTPS pages. On a plaintext page the bigger story
	// is the page itself, surfaced by other checks (security-headers HSTS,
	// mixed-content).
	if !strings.HasPrefix(strings.ToLower(p.URL), "https://") {
		return nil, nil
	}

	snap, err := ensureResponse(ctx, client, p, formActionInsecureBodyCap)
	if err != nil {
		return nil, err
	}
	...
	forms, candidates := c.parse(snap.Body, pageURL)

	var findings []Finding
	seen := map[string]struct{}{}

	for _, cand := range candidates {
		if !strings.EqualFold(cand.resolved.Scheme, "http") {
			continue
		}

		key := cand.resolved.String()
		dedupeKey := MakeKey(c.Name(), ScopePage, p.URL, "action:"+key)
		...
		if cand.formIdx >= 0 && cand.formIdx < len(forms) {
			inputs = forms[cand.formIdx].inputs
		}
		severity, hasCredentialField := classifySeverity(inputs)
		...
		findings = append(findings, Finding{
			...
			URL:         p.URL,
			Severity:    severity,
			Title:       buildTitle(hasCredentialField, cand.override),
			Detail:      buildDetail(p.URL, cand, key, inputs, hasCredentialField),
			CWE:         "CWE-319",
			OWASP:       "A02:2021 Cryptographic Failures",
			Remediation: buildRemediation(cand.method),

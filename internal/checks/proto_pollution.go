...
func (ProtoPollution) Name() string { return "proto-pollution" }

func (ProtoPollution) Level() Level { return LevelAggressive }

// Budget grants room for baseline observer + per-sink (pollute +
// observe + cleanup) sequences across a page that may expose several
// probable sinks. Three requests per sink * a handful of sinks plus
// the per-page baseline keeps DefaultBudget (60s) tight under any
// rate limiter; three minutes covers it without pinning a worker
// slot on a pathological page.
func (ProtoPollution) Budget() time.Duration { return 3 * time.Minute }
...
func (c ProtoPollution) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) ([]Finding, error) {
	...
	// the page URL is a fine witness regardless of which sink we
	// probe; sharing the baseline across sinks halves the observer
	// cost on pages with several probable inputs.
	baseObs, err := c.observe(ctx, client, p.URL)
	if err != nil {
		Report(ctx, fmt.Errorf("proto-pollution baseline observer %s: %w", p.URL, err))
		return nil, err
	}
	...
	for _, sink := range sinks {
		...
		f, err := c.probe(ctx, client, p.URL, sink, baseObs)
		if err != nil {
			Report(ctx, fmt.Errorf("proto-pollution %s %s=%s: %w", sink.Loc, sink.Name, sink.URL, err))
			if firstErr == nil {
				firstErr = err
			}
			continue
...
func (c ProtoPollution) probe(ctx context.Context, client *httpclient.Client, pageURL string, sink Sink, baseObs ppObservation) (*Finding, error) {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopeParam, pageURL, "loc:"+string(sink.Loc), "param:"+sink.Name),
	}, nil
}

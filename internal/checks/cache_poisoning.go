...
func (CachePoisoning) Name() string { return "cache-poisoning" }

func (CachePoisoning) Level() Level { return LevelDefault }
...
func (c CachePoisoning) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) ([]Finding, error) {
	...
	base, err := ensureResponse(ctx, client, p, cachePoisonBodyCap)
	if err != nil {
		return nil, nil
	}
	if base.Status < 200 || base.Status >= 400 {
		return nil, nil
	}
	...
	// Unkeyed-header arm: only run when the baseline carries a cache
	// indication. Without an intermediary cache in the path, the worst
	// case is per-request reflection (covered by host-header-injection)
	// rather than a stored poison that hits every later victim.
	if cacheHintsPresent(base.Headers) {
		varyHeaders := parseVary(base.Headers.Get("Vary"))
		for _, probe := range cachePoisonHeaderProbes() {
			if ctx.Err() != nil {
				break
			}
			f, err := c.probeUnkeyedHeader(ctx, client, p.URL, probe, base, varyHeaders)
			if err != nil {
				Report(ctx, fmt.Errorf("cache-poisoning header %s: %w", probe.name, err))
				if firstErr == nil {
					firstErr = err
				}
				continue
	...
	// Cache-deception arm: run on paths that look authentication-bearing,
	// or every page when at LevelAggressive. The bug is at the server -
	// suffix-confused routing - so the absence of a cache hint on the
	// baseline doesn't disprove exploitability; a downstream CDN with
	// extension-based rules will pick it up.
	if isAuthLikelyPath(u.Path) || LevelFrom(ctx) >= LevelAggressive {
		f, err := c.probeCacheDeception(ctx, client, p.URL, u, base)
		if err != nil {
			Report(ctx, fmt.Errorf("cache-deception probe: %w", err))
			if firstErr == nil {
				firstErr = err
			}
		} else if f != nil {
...
func (c CachePoisoning) probeUnkeyedHeader(ctx context.Context, client *httpclient.Client, target string, probe unkeyedHeader, base snapshot, vary map[string]struct{}) (*Finding, error) {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopeHost, target, "unkeyed-header", "name:"+strings.ToLower(probe.name)),
	}, nil
}
...
func (c CachePoisoning) probeCacheDeception(ctx context.Context, client *httpclient.Client, target string, u *url.URL, base snapshot) (*Finding, error) {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopePage, target, "cache-deception"),
	}, nil
}

...
func (CSPWeak) Name() string { return "csp-weak" }

func (CSPWeak) Level() Level { return LevelPassive }
...
func (c CSPWeak) Run(ctx context.Context, client *httpclient.Client, _ *scope.Scope, p page.Page) ([]Finding, error) {
	...
	reportOnly := snap.Headers.Values("Content-Security-Policy-Report-Only")
	if len(enforcing) == 0 && len(reportOnly) == 0 {
		// Absence is handled by security-headers; nothing for us to say.
		return nil, nil
	}

	// Choose the policy to analyze. An enforcing header is preferred; an
	// origin running Report-Only only is flagged separately so the reviewer
	// knows nothing is actually blocking. When multiple CSP headers are
	// present the browser intersects them; we analyze the first enforcing
	// one as the dominant policy and note the multi-header case in details.
	// When both an enforcing and a Report-Only header are sent, the Report-
	// Only one is deliberately ignored here: it isn't what the browser blocks
	// on, and a parallel stricter staged policy is the author's WIP rather
	// than a defect for this check to flag.
	var (
		policyHeader string
		isReportOnly bool
	)
	...
	for _, w := range weaknesses {
		...
		details = append(details, fmt.Sprintf("%s [%s]: %s", w.directive, w.severity, w.detail))
		idParts = append(idParts, w.directive+":"+w.id)
	}
	...
	if len(weaknesses) == 1 {
		...
		title = fmt.Sprintf("Content-Security-Policy has %d weaknesses", len(weaknesses))
	}
	if isReportOnly {
		title += " (Report-Only)"
	}

	leadIn := fmt.Sprintf("Response from %s ships a Content-Security-Policy but the policy contains directives that materially reduce or eliminate its XSS / framing protection. Each entry below names the weak directive, the resulting risk, and how to tighten it.", p.URL)

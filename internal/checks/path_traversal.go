...
func (PathTraversal) Level() Level { return LevelDefault }
...
func (c PathTraversal) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) ([]Finding, error) {
	...
	sweep := LevelFrom(ctx) >= LevelAggressive
	...
	for _, sink := range sinks {
		...
		if !sweep && !pathSinkCandidate(sink) {
			continue
		}

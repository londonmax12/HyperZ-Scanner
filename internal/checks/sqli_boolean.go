...
func (SQLiBoolean) Name() string { return "sqli-boolean" }

func (SQLiBoolean) Level() Level { return LevelDefault }
...
func (c SQLiBoolean) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) ([]Finding, error) {
	...
	// Surface the first sink failure only when no sink completed at all -
	// otherwise "no findings" is a real negative result, not a check
	// failure, and per-sink errors have already been delivered via Report.

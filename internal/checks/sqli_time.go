...
func (SQLiTime) Name() string { return "sqli-time" }

func (SQLiTime) Level() Level { return LevelDefault }

// Budget overrides DefaultBudget. Worst case per page: every sink takes
// 1 fast baseline + N fast non-matching payloads, plus one or two sinks
// that exercise the confirmation path at ~2*sleepFor wall time. Five
// minutes covers a wide page where a couple of sinks confirm without
// pinning a worker slot on a slow target.
func (SQLiTime) Budget() time.Duration { return 5 * time.Minute }
...
func (c SQLiTime) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) ([]Finding, error) {

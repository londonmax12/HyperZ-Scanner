...
func (c *OpenAPIAudit) Name() string { return "openapi-audit" }
...
func (c *OpenAPIAudit) findingEmbeddedCredentials(f *OpenAPIAuditFacts) *Finding {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopeHost, probeURL, append([]string{"embedded-credentials"}, idParts...)...),
	}
}
...
func (c *OpenAPIAudit) findingExampleAuthTokens(f *OpenAPIAuditFacts) *Finding {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopeHost, probeURL, append([]string{"example-auth-tokens"}, idParts...)...),
	}
}
...
func (c *OpenAPIAudit) findingAuthlessOperations(f *OpenAPIAuditFacts) *Finding {
	...
	return &Finding{
		...
		DedupeKey: MakeKey(c.Name(), ScopeHost, probeURL, append([]string{"authless-operations"}, idParts...)...),
	}
}

...
//	  checks.BooleanCompare verbatim so the Lua-port verdict matches
...
//	  Wraps checks.TimingCompare. Lua uses seconds-as-float-doubles
...
//	  Wraps checks.Similarity. Lets a check show the two contributing
...
func readSnapshotArg(v lua.LValue) checks.Snapshot {
	t, ok := v.(*lua.LTable)
	if !ok {
...
func oracleBooleanCompare(L *lua.LState) int {
	...
	res := checks.BooleanCompare(baseline, truthy, falsy)
	out := L.NewTable()
...
func oracleSimilarity(L *lua.LState) int {
	a := requireString(L, 1)
	b := requireString(L, 2)
	L.Push(lua.LNumber(checks.Similarity([]byte(a), []byte(b))))
	return 1
}

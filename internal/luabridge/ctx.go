...
func buildCtxUserdata(L *lua.LState, env *runEnv) *lua.LTable {
	...
	t.RawSetString("level", lua.LString(checks.LevelFrom(env.ctx).String()))
...
func ctxEnsureResponse(L *lua.LState) int {
	...
	snap, err := callEnsureResponse(env, maxBody)
	if err != nil {
		L.Push(lua.LNil)
...
func ctxReport(L *lua.LState) int {
	...
	checks.Report(env.ctx, errors.New(msg))
	return 0
}
...
func ctxLevelAtLeast(L *lua.LState) int {
	...
	want, err := checks.ParseLevel(requireString(L, 2))
	if err != nil {
		L.ArgError(2, err.Error())
	}
	L.Push(lua.LBool(checks.LevelFrom(env.ctx) >= want))
	return 1
}

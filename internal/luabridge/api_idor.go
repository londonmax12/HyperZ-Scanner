...
// scan-lifetime *checks.Corpus alive on the LuaCheck via AuxOrCreate
// so cross-page ingest survives between Run calls the same way the Go
// check's `&checks.IDOR{}` registration does. The Lua side gets a
...
// idorCorpusKey identifies the per-LuaCheck slot the *checks.Corpus
// lives in. Zero-size unique type so AuxOrCreate's map cannot collide
// with another helper's key.
type idorCorpusKey struct{}

// corpusUserData wraps the *checks.Corpus the active LuaCheck owns
...
// findPatternByName resolves a pattern name to its *checks.Pattern,
...
func findPatternByName(c *checks.Corpus, name string) *checks.Pattern {
...
// idorJudgeFn mirrors checks.IDORJudge: takes baseline / tampered /
...
func idorJudgeFn(L *lua.LState) int {
	...
	v := checks.IDORJudge(baseline, tampered, control)
	out := L.NewTable()
...
func idorControlPayload(L *lua.LState) int {
	...
	L.Push(lua.LString(checks.IDORControlPayload(pat, seed)))
	return 1
}

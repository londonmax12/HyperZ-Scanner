...
//
// The per-LuaCheck *checks.JWTVulns instance is held via AuxOrCreate
...
// check's `&checks.JWTVulns{}` registration does. Same pattern as
...
// *checks.JWTVulns lives in. Unique zero-size type per AuxOrCreate
// convention.
...
func jwtFactsToLua(L *lua.LState, facts []checks.JWTFact) *lua.LTable {
	out := L.NewTable()
	for i, f := range facts {
		entry := L.NewTable()

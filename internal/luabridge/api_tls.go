...
//
//	ctx.tls.now()
//	  -> unix_int seconds. Routes through checks.TLSAuditNowUnix so a
...
// through checks.TLSAuditHandshakeLua which dials through the same
...
func tlsHandshake(L *lua.LState) int {
	...
	res, err := checks.TLSAuditHandshakeLua(env.ctx, addr, serverName)
	if err != nil {
		L.Push(lua.LNil)
...
func tlsNow(L *lua.LState) int {
	L.Push(lua.LNumber(checks.TLSAuditNowUnix()))
	return 1
}

func tlsDialTimeoutSeconds(L *lua.LState) int {
	L.Push(lua.LNumber(checks.TLSAuditDialTimeoutSeconds()))
	return 1
}

func tlsExpiryUrgentWindowSeconds(L *lua.LState) int {
	L.Push(lua.LNumber(checks.TLSAuditExpiryUrgentWindowSeconds()))
	return 1
}

func tlsExpirySoonWindowSeconds(L *lua.LState) int {
	L.Push(lua.LNumber(checks.TLSAuditExpirySoonWindowSeconds()))
	return 1
}
...
func tlsCertHasEmbeddedSCT(L *lua.LState) int {
	c := tlsCertFromArg(L).c
	L.Push(lua.LBool(checks.TLSAuditCertEmbedsSCT(c)))
	return 1
}
...
func tlsCertVerifiesHostname(L *lua.LState) int {
	...
	L.Push(lua.LBool(checks.TLSAuditCertVerifyHostname(c, host)))
	return 1
}

...
//	                                  checks.NewCanary so Lua authors
...
// browserNewCanary wraps checks.NewCanary. Returns the same per-call
...
func browserNewCanary(L *lua.LState) int {
	L.Push(lua.LString(checks.NewCanary()))
	return 1
}
...
func browserAttached(L *lua.LState) int {
	...
	L.Push(lua.LBool(checks.BrowserFrom(env.ctx) != nil))
	return 1
}
...
// the Go check threads them through checks.Report.
func browserVisit(L *lua.LState) int {
	...
	pool := checks.BrowserFrom(env.ctx)
	if pool == nil {
		L.Push(lua.LBool(false))
		L.Push(lua.LNil)
		return 2

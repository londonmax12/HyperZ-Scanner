...
// LuaCheck adapts a compiled Lua module to the checks.Check interface.
...
func readCheckMeta(t *lua.LTable) (checkMeta, error) {
	...
	lvl, err := checks.ParseLevel(levelStr)
	if err != nil {
...
// Level satisfies checks.Check.
func (c *LuaCheck) Level() checks.Level { return c.level }
...
// (DefaultBudget). The checks.Budgeted interface is satisfied
...
func (c *LuaCheck) Run(ctx context.Context, client *httpclient.Client, sc *scope.Scope, p page.Page) (findings []checks.Finding, runErr error) {
	L, mod, err := c.borrow()
	if err != nil {
		return nil, err
	}
...
// parseScope mirrors checks.ParseSeverity for the Scope enum. Kept
...
func parseScope(s string) (checks.Scope, error) {
	switch s {
	case "host":
		return checks.ScopeHost, nil
	case "page":
		return checks.ScopePage, nil
	case "param":
		return checks.ScopeParam, nil
	}

...
func deserialFormats(L *lua.LState) int {
	out := L.NewTable()
	for i, f := range checks.DeserialFormatListLua() {
		entry := L.NewTable()
...
func deserialClassify(L *lua.LState) int {
	name, label := checks.DeserialClassifyValueLua(requireString(L, 1))
...
func deserialMatchAll(L *lua.LState) int {
	hits := checks.DeserialMatchAllLua([]byte(requireString(L, 1)))
	out := L.NewTable()
	for i, h := range hits {
...
func deserialMatchFormat(L *lua.LState) int {
	...
	hits := checks.DeserialMatchFormatLua([]byte(body), name)
	out := L.NewTable()
	for i, h := range hits {
		out.RawSetInt(i+1, lua.LString(h))
	}
...
func deserialBodyMarker(L *lua.LState) int {
	L.Push(lua.LString(checks.DeserialBodyMarkerLua([]byte(requireString(L, 1)))))
	return 1
}

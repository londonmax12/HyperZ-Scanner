...
// redirect-related).
//
// Sinks come back as userdata wrapping *checks.Sink so Lua-side
...
//
// Output is sorted with the same comparator checks.SinksFor uses so
...
func sinksForPage(L *lua.LState) int {
	...
	out := make([]checks.Sink, 0, len(base))
	for _, s := range base {
...
// sinkUserData wraps a *checks.Sink. Pointer wrap rather than value
...
func pushSink(L *lua.LState, s *checks.Sink) lua.LValue {
	ud := L.NewUserData()

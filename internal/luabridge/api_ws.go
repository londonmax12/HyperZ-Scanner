...
func buildWSTable(L *lua.LState) *lua.LTable {
	...
	t.RawSetString("max_endpoints_per_page", L.NewFunction(wsMaxEndpointsPerPage))
	return t
}
...
func wsDiscoverEndpoints(L *lua.LState) int {
	...
	out := L.NewTable()
	for i, ep := range checks.WSAuditDiscoverEndpointsLua([]byte(body)) {
		out.RawSetInt(i+1, lua.LString(ep))
	}
	L.Push(out)
	return 1
}
...
func wsHandshakeBinding(L *lua.LState) int {
	...
	res, err := checks.WSAuditHandshakeLua(env.ctx, url, origin)
	if err != nil {
		L.Push(lua.LNil)
...
func wsForeignOrigin(L *lua.LState) int {
	L.Push(lua.LString(checks.WSAuditForeignOriginLua()))
	return 1
}

func wsMaxEndpointsPerPage(L *lua.LState) int {
	L.Push(lua.LNumber(checks.WSAuditMaxEndpointsPerPageLua()))
	return 1
}

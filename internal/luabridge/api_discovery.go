...
func discoveryEntries(L *lua.LState) int {
	...
	stack := checks.StackFrom(env.ctx)
	out := L.NewTable()
	for i, e := range checks.ContentDiscoveryEntriesLua(aggressive, hostname, stack) {
...
func discoveryFollowUps(L *lua.LState) int {
	...
	stack := checks.StackFrom(env.ctx)
	out := L.NewTable()
	for i, e := range checks.ContentDiscoveryFollowUpsLua(hits, probed, stack) {
		out.RawSetInt(i+1, pushDiscoveryEntry(L, e))
	}
	L.Push(out)
	return 1
...
func pushDiscoveryEntry(L *lua.LState, e checks.ContentDiscoveryEntryLua) *lua.LTable {
	t := L.NewTable()
...
func discoveryBodyHashPrefix(L *lua.LState) int {
	L.Push(lua.LString(checks.ContentDiscoveryBodyHashPrefixLua([]byte(requireString(L, 1)))))
	return 1
}

func discoveryContentTypeFamily(L *lua.LState) int {
	L.Push(lua.LString(checks.ContentDiscoveryContentTypeFamilyLua(requireString(L, 1))))
	return 1
}
...
func discoveryContentTypeFamilyAllowed(L *lua.LState) int {
	...
	L.Push(lua.LBool(checks.ContentDiscoveryContentTypeFamilyAllowedLua(ct, allowed)))
	return 1
}
...
func discoveryLengthCloseTo(L *lua.LState) int {
	a := L.CheckInt(1)
	b := L.CheckInt(2)
	L.Push(lua.LBool(checks.ContentDiscoveryLengthCloseToLua(a, b)))
	return 1
}

func discoveryCanaryPath(L *lua.LState) int {
	L.Push(lua.LString(checks.ContentDiscoveryCanaryPathLua()))
	return 1
}

func discoveryBaselineProbes(L *lua.LState) int {
	L.Push(lua.LNumber(checks.ContentDiscoveryBaselineProbes()))
	return 1
}

func discoveryBodyCap(L *lua.LState) int {
	L.Push(lua.LNumber(checks.ContentDiscoveryBodyCap()))
	return 1
}

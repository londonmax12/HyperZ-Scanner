...
local function batch_accepted(ctx, body)
  ...
  -- Array length check via Len; gopher-lua's # operator follows the
  -- same contiguous-1..n rule as gopher-lua's json decoder produces.
  local n = #arr
...
local function batch_mutations_executed(ctx, body, field)
  local arr = ctx.json.decode(body)
  if type(arr) ~= "table" or #arr < 2 then return false end
  local executed = 0
  for i = 1, #arr do
    local elem = arr[i]
    if type(elem) ~= "table" then return false end

...
function check.run(ctx)
  ...
  for _, ep in ipairs(endpoints) do
    ...
    if not eperr and ep_url and ep_url.host ~= "" then
      ...
      -- Same-host filter: we don't own third-party endpoints; probing
      -- them is out-of-scope by default. Scope is re-checked too so a
      -- same-host endpoint outside the operator's scope (e.g. port-
      -- restricted) is not probed. Probe cap matches the Go check.
      local same_host = string.lower(ep_url.hostname) == string.lower(page_url.hostname)
      if same_host and ctx.scope:allows(ep) and probed < probe_cap then
        probed = probed + 1

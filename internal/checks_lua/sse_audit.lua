...
local function build_sse_snippet(status, headers, body)
  local lines = {}
  lines[#lines + 1] = string.format("HTTP/1.1 %d %s", status, ctx_status_text(status))
...
-- ctx_status_text is set inside check.run because the body helpers
-- table only exists once we hold ctx. Wrap the static helper to keep
-- the snippet builder above context-clean.
ctx_status_text = function(_) return "" end
...
local function probe_endpoint(ctx, target)
  ...
  return {
    ...
    evidence = ctx.evidence.from_exchange {
      ...
      body      = body,
      truncated = truncated,
      snippet   = build_sse_snippet(resp:status(), resp:headers(), body),
    },
...
function check.run(ctx)
  ctx_status_text = function(code) return ctx.body.status_text(code) end
  ...
  for _, ep in ipairs(endpoints) do
    ...
    if ep_u ~= nil and ep_u.host ~= ""
       and ep_u.hostname:lower() == page_u.hostname:lower()
       and ctx.scope:allows(ep) then
      probed = probed + 1

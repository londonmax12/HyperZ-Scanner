...
// DrainFacts is the OOB-pass counterpart of ScanFacts: one JWTFact per
// OOB callback the JWT probes triggered during the active phase. The
// scanner calls this after the wait window through the Lua bridge so
// findings still flow for tokens whose validator dereferenced the
// canary URL out of band. Each registered canary that received at
...
func (c *JWTVulns) DrainFacts(ctx context.Context) []JWTFact {
	...
	regs := srv.Registrations("jwt-vulns")
	if len(regs) == 0 {
		return nil
	}
	var out []JWTFact
	for _, reg := range regs {
		...
		finding := buildJWTKeyURLOOBFinding(reg, hits)
		out = append(out, factFromFinding(finding.Target, finding))
	}
	return out
}

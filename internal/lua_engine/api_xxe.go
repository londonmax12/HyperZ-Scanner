...
func buildXXETable(L *lua.LState) *lua.LTable {
	t := L.NewTable()
	t.RawSetString("file_disclose_docs", L.NewFunction(xxeFileDiscloseDocsFn))
	t.RawSetString("error_docs", L.NewFunction(xxeErrorDocsFn))
	t.RawSetString("baseline_doc", L.NewFunction(xxeBaselineDocFn))
	...
	t.RawSetString("oob_exfil_probe_file", L.NewFunction(xxeOOBExfilProbeFileFn))
...
func xxeBaselineDocFn(L *lua.LState) int {
	L.Push(lua.LString(XXEBaselineDocLua()))
	return 1
}
...
func xxeOOBExfilProbeFileFn(L *lua.LState) int {
	L.Push(lua.LString(XXEOOBExfilProbeFileLua()))
	return 1
}

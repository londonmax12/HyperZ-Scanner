...
// strings rather than ints because Severity is a string type
// and there's nothing to be gained by interposing a numeric layer.
func buildSeverityTable(L *lua.LState) *lua.LTable {
	t := L.NewTable()
	t.RawSetString("info", lua.LString(SeverityInfo))
	t.RawSetString("low", lua.LString(SeverityLow))
	t.RawSetString("medium", lua.LString(SeverityMedium))
	t.RawSetString("high", lua.LString(SeverityHigh))
	t.RawSetString("critical", lua.LString(SeverityCritical))
	return t
}

// buildScopesTable mirrors the Scope enum for dedupe-key
...
// buildLevelsTable exposes the Level vocabulary. The active
...
// buildLocsTable exposes the Loc enum used by Sink. Lua
// authors that read sink.loc compare against ctx.locs.query,
// ctx.locs.form, etc. The string values are exactly the wire form
// Loc carries so a sink loc surfaces as the same string in
// Lua and in Go.
func buildLocsTable(L *lua.LState) *lua.LTable {
	t := L.NewTable()
	t.RawSetString("query", lua.LString(LocQuery))
	t.RawSetString("form", lua.LString(LocForm))
	t.RawSetString("header", lua.LString(LocHeader))
	t.RawSetString("cookie", lua.LString(LocCookie))
	t.RawSetString("json", lua.LString(LocJSON))
	t.RawSetString("path", lua.LString(LocPath))
	return t
}
